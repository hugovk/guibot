...
def train(model, hyperparams, train_samples, train_targets, epoch, data_filename=None):
    ...
    optimizer = optim.SGD(model.parameters(),
                          lr=hyperparams["learning_rate"].value,
                          momentum=hyperparams["sgd_momentum"].value)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % hyperparams["log_interval"].value == 0:
...
for epoch in range(1, EPOCHS + 1):
    ...
    train(finder.model, hyperparams, 'samples_train.pth', 'targets_train.pth', epoch, NEEDLE)
    # test trained network on test samples
    test(finder.model, hyperparams, 'samples_test.pth', 'targets_test.pth')
